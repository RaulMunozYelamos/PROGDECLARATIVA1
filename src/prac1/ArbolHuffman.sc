...
trait ArbolHuffman {
  ...
  */
  ...
  def codificar(cadena:String,arbol:ArbolHuffman):List[Bit] = {
    @tailrec
      def codificaraux(caracter: List[Char], listavacia: List[Bit], arbolA: ArbolHuffman): List[Int] = arbolA match {
      case RamaHuff(nodoizq, nododch) if caracter!=Nil => if nododch.buscar(caract.head) then codificaraux(nododch, bits:+1, caracter) else if nodoizq.buscar(caracter.head) then codificaraux(nodoizq,bits:+0, caracter) else throw new Exception("ERROR")
      case HojaHuff(caracter, pesoHoja) if caracter!=Nil => codificaraux(this, listavacia, caracter.tail)
      case _ => listavacia


    }
    codificaraux(this, List())
  }

  cadenaAListChars(cadena)
